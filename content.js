...
function copiarNumero() {
    ...
    var spanElement = document.querySelector("#main > header > div._amie > div > div > div > span");
    ...
    if (spanElement) {
        ...
        if (/^\+\d{1,3}\s\d{3}\s\d{6,}$/.test(searchText)) {
            ...
            window.open('https://contacts.google.com/search/' + encodeURIComponent(searchText), '_blank');
        }
    } else {
